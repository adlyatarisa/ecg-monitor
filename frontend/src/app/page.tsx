    ...
    <main className="min-h-screen bg-white font-sans p-6 md:p-8 flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ChartCard
          ...
          live
          data={healthyBuffer}
          options={buildChartOptions(undefined, undefined)}
          className="h-[300px]"
        />
        <FFTChartCard
          title="Healthy FFT"
      ...
      <div className="bg-[#f0f9ff] border border-[#bfdbfe] rounded-2xl p-6">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
          <div>
            <p className="text-[12px] font-bold text-[#1e3a8a] uppercase tracking-wider">Frequency-Based Stroke Detection</p>
            <p className="text-[10px] font-semibold text-[#3b82f6] mt-1">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[11px]">
            <thead>
              <tr className="text-left text-[#1e3a8a] uppercase tracking-wider">
            ...
            <tbody>
              {strokeDetection.checks.length === 0 ? (
                <tr><td colSpan={7} className="py-3 text-[#93c5fd] font-semibold">Waiting for reference data...</td></tr>
              ) : strokeDetection.checks.map((c, idx) => {
                const hit = c.ecgHit || c.pcgHit;
                return (
                  <tr key={idx} className="border-t border-[#bfdbfe] text-[#1e3a8a]">
